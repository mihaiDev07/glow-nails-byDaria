...
const images = Array.from({ length: 9 }, (_, index) => ({ src: `/img/nails${index + 1}.jpeg`, alt: `Model de unghii ${index + 1}` }));
const PER_PAGE = 8;
  ...
  return (
    <>
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {visible.map((image, index) => <div className="group relative h-80 overflow-hidden rounded-xl border border-white/80 bg-white shadow-[0_.3rem_1rem_rgba(45,30,40,.09)] transition duration-500 hover:-translate-y-2 hover:scale-[1.015] hover:shadow-[0_24px_50px_rgba(75,42,62,.2)] animate-[cardRise_.65s_ease-out_both]" style={{ animationDelay: `${index * 70}ms` }} key={image.src}><Image className="h-full w-full object-cover transition duration-700 group-hover:scale-110 group-hover:saturate-110" src={image.src} alt={image.alt} width={700} height={700} /><span className="pointer-events-none absolute inset-0 bg-gradient-to-br from-white/25 to-transparent opacity-0 transition duration-500 group-hover:opacity-100" /></div>)}
      </div>
