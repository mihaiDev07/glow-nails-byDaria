  ...
  return (
    <header className="relative z-20 bg-gradient-to-r from-[#643f6d] via-[#b5658e] to-[#efd1c2] px-4 text-white shadow-[0_7px_24px_rgba(75,40,65,.2)] animate-[headerDrop_.65s_ease-out_both]">
      <nav className="mx-auto flex min-h-32 w-full max-w-7xl items-center justify-between py-2 md:min-h-40" aria-label="Navigație principală">
        ...
        <button className="relative z-50 flex h-13 w-13 flex-col items-center justify-center gap-1.5 rounded-xl border border-white/80 bg-[#58335f]/90 shadow-[0_6px_18px_rgba(52,26,52,.38)] backdrop-blur-sm transition hover:bg-[#48284e] md:hidden" type="button" aria-label={open ? "Închide meniul" : "Deschide meniul"} aria-expanded={open} onClick={() => setOpen(!open)}>
          <span className={`h-[3px] w-7 rounded-full bg-white shadow-sm transition duration-300 ${open ? "translate-y-[9px] rotate-45" : ""}`} />
          <span className={`h-[3px] w-7 rounded-full bg-white shadow-sm transition duration-300 ${open ? "opacity-0" : ""}`} />
          <span className={`h-[3px] w-7 rounded-full bg-white shadow-sm transition duration-300 ${open ? "-translate-y-[9px] -rotate-45" : ""}`} />
        </button>
